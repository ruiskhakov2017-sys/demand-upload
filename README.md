...
**Private Google Ads Analytics & Control Center** — серверная платформа для
централизованного сбора, нормализации, анализа и операционного контроля данных
из нескольких MCC и рекламных аккаунтов. Проект разрабатывает и использует
Iskhakov Ruslan как единственный владелец и оператор; это не публичный SaaS и
не сервис для внешних клиентов.
...
Рабочая версия: [https://axyro.tech](https://axyro.tech)

## Быстрый маршрут для ревью

Перед чтением всего репозитория откройте
[краткий обзор для технического ревью](docs/mentor-review.md). В нём описаны
архитектура, основные потоки, карта ключевых модулей, порядок запуска и вопросы,
по которым особенно полезна обратная связь.

Основные документы:

- [архитектура](docs/architecture.md);
- [устройство Google Ads adapter](docs/adr/0002-google-ads-adapter.md);
- [аутентификация и хранение секретов](docs/adr/0003-auth-and-secrets.md);
- [пользовательская инструкция](docs/user-guide-ru.md);
- [итоговая приёмка](docs/final-acceptance-report.md).
